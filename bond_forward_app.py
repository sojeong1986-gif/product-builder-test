...
def load_data(path="bond_forward.xlsx"):
    ...
    for _, row in df_raw.iterrows():
        ...
        # 잔액: col index 3~8 (텍스트 포함, 쉼표/공백 제거)
        balances = []
...
try:
    ...
    st.error("❌ `bond_forward.xlsx` 파일을 찾을 수 없습니다. GitHub 레포에 파일이 있는지 확인해주세요.")
    st.stop()
...
if prev_idx >= 0:
    prev_bal = df_long[df_long["기간"]==PERIODS[prev_idx]]["잔액"].sum()
...
with table_col:
    ...
    def render_group(구분):
        ...
        rows_html += f'<tr class="group-total"><td>{"손보" if 구분=="손보" else "생보"} 합계</td><td></td><td>{grp_bal:,.0f}</td><td></td><td>{share:.1f}%</td></tr>'
        return rows_html
...
for 구분, bg, fg, label in [("손보","#e3f2fd","#1565c0","손해보험"),("생보","#f3e5f5","#6a1b9a","생명보험")]:
    ...
    gcells = "".join([f'<td style="{"background:#fff8e1;" if p==sel_period else ""}">{s:,.0f}</td>' for p,s in zip(PERIODS, grp_sums)])
